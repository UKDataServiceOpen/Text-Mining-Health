...
install.packages("textstem") # lemmitisation
...
token_list <- foot_mouth_df %>% unnest_tokens(output = tokenised_words, 
                   token = "words",
...
We can use the hunspell_check and hunspell_suggest functions from the *hunspell* package to test indivial words


```{r}
#foot_mouth_df$spell_checked <-
#hunspell(foot_mouth_df$Everything_else[2,])
...
foot_mouth_df$stem_words <- 
  stemDocument(foot_mouth_df$no_stop_words, language = "english")
...
```{r}
foot_mouth_df$lemm <- 
  lemmatize_words(foot_mouth_df$pos_tags)

head(foot_mouth_df$lemm)
